...
def load_store_dimension(directory: str) -> pd.DataFrame:
    ...
    for file in all_files:
        try:
            ...
            df = pd.read_csv(
                file, skiprows=7, dtype=str, encoding="utf-8"
...
if __name__ == "__main__":
    FILE_PATH = (
        "C:/Customer-Promo-Analytics/data/sobeys_stores"  # Change to your file path
    )
    CREDENTIALS_PATH = (
        "C:/.keys/keyfilepromo.json"  # Update with your credentials file path
    )
    PROJECT_ID = "customer-promo"
    DATASET_ID = "raw"
    TABLE_ID = "raw_sobeys_stores"
