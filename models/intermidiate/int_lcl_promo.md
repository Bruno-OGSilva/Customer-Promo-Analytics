# Intermediate Model: int_lcl_promo

## Purpose
Identify promotional weeks and promo depth (% off) for Loblaw at **banner + UPC + week**.

## Inputs
- `stg_lcl_sales`: POS fact at store level.
- `int_lcl_stores`: store attributes with `banner` and `retail_group`.
