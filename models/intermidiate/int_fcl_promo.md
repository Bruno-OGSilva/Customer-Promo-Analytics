# Intermediate Model: int_fcl_promo

## Purpose
Identify promotional weeks and promo depth (% off) for Federated Coop at **banner + UPC + week**.

## Inputs
- `stg_fcl_sales`: POS fact at store level.
- `int_fcl_stores`: store attributes with `banner` and `retail_group`.
