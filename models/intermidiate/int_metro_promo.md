# Intermediate Model: int_metro_promo

## Purpose
Identify promotional weeks and promo depth (% off) for Metro at **banner + UPC + week**.

## Inputs
- `stg_metro_sales`: POS fact at store level.
- `int_metro_stores`: store attributes with `banner` and `retail_group`.
