# Intermediate Model: int_ccl_promo

## Purpose
Identify promotional weeks and promo depth (% off) for Calgary Coop at **banner + UPC + week**.

## Inputs
- `stg_ccl_sales`: POS fact at store level.
- `int_ccl_stores`: store attributes with `banner` and `retail_group`.
