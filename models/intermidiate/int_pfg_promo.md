# Intermediate Model: int_pfg_promo

## Purpose
Identify promotional weeks and promo depth (% off) for Pattison Food Group at **banner + UPC + week**.

## Inputs
- `stg_pfg_sales`: POS fact at store level.
- `int_pfg_stores`: store attributes with `banner` and `retail_group`.
